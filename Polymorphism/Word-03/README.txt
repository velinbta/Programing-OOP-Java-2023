...
The program reads a line of text from the console.
Then starts reading commands for editing (text-transform) and executing them on the text.
...
All commands are of the form: "{commandName} {startIndex} {endIndex}". 
...
E.g. if the current text is som3. text and we execute the command cut 1 7:
...
Note: if startIndex == endIndex, the paste will insert the text at position startIndex.
Meaning that any text at startIndex will be pushed to the right by the inserted text. E.g.
If the last command was paste 0 0 (not paste 3 4), the text would be om3. Tsext.
...
(just like in any text editor - you can cut something and paste it several times).
